...
//at most one share of the stock at any time. However, you can buy it then immediately sell it on the same day.
...
int maxProfit(List<int> prices) {
  int n = prices.length;
  List<List<int>> dp = List.generate(n, (_) => List.filled(2, -1));
...
int maxProfitWithoutDP(List<int> prices) {
  int f(int ind, int buy, List<int> values, int n) {
    print(ind);
    if (ind == n) return 0;
    ...
    if (buy == 1) {
      ...
      profit = max(
          values[ind] + f(ind + 1, 1, values, n), 0 + f(ind + 1, 0, values, n));
    }
    return profit;
  }
